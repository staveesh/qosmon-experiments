...
def go():
    experiment_id = 'experiment_{}_{}'.format(6, 10)
    ...
    for algorithm in algorithms:
        # Server side
        print('#' * 40)
        print('Configuration : n_devices = {} jtd = {} algo = {}'.format(6, 10, algorithm.upper()))
        ...
        sftp.put(os.path.join(os.getcwd(), 'topos/topo-{}.json'.format(6)),
                 '/home/' + containernet['username'] + '/topo-{}.json'.format(6))
        sleep(10)
        sftp.close()
        print('Initiating python script....')
        stdin, stdout, stderr = session.exec_command(
            "sudo -S -p '' python topology.py {}".format('topo-{}.json'.format(6)))
